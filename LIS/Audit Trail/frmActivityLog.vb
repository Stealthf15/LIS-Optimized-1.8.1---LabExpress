...
Public Class frmActivityLog
    ...
    Public Sub LoadRecords()
        Try
            ...
            '            `user` AS User, `activity` AS Activity, `test` AS Request, `date` AS Date
            '            FROM `z_logs_activity` WHERE (DATE_FORMAT(`date`, '%Y-%m-%d') BETWEEN @DateFrom and @DateTo) ORDER BY `date` DESC"

            Dim SQL As String = "SELECT 
                        `id` AS ID, `sample_id` AS SampleID, `patient_id` AS PatientID, `name`AS PatientName, 
                        `user` AS User, `activity` AS Activity, `test` AS Request, DATE_FORMAT(`date`, '%m/%d/%Y %r') AS Date
                        FROM `z_logs_activity` WHERE (DATE_FORMAT(`date`, '%Y-%m-%d') BETWEEN @DateFrom and @DateTo) ORDER BY `id` DESC"
