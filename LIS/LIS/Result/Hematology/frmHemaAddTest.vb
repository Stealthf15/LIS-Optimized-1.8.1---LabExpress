...
Public Class frmHemaAddTest
    ...
    Public Sub LoadRecords()
        Try
            ...
            Connect()
            rs.Connection = conn
            'rs.CommandText = "SELECT * FROM `default_specimen` WHERE `section` = 'Hematology' AND `test_name` = 'CBC' AND `test_group` = 'Differential Count' AND `status` = 'Enable' ORDER BY `order_no`"
            rs.CommandText = "SELECT * FROM `specimen` WHERE `section` = 'Hematology' AND `status` = 'Enable' AND `instrument` = 'Other_Test' ORDER BY `order_no`"
            reader = rs.ExecuteReader
            While reader.Read()
                If reader.HasRows Then
                    iItem = New ListViewItem(reader(0).ToString, 0)
                    'For x As Integer = 1 To 15 Step 1
                    For x As Integer = 1 To lvList.Columns.Count - 1 Step 1
    ...
    Private Sub btnAddTest_ItemClick(ByVal sender As Object, ByVal e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnAddTest.ItemClick
        ...
        'For x As Integer = 0 To .Rows.Count To 0 Step -1
        '    For y = Me.lvList.CheckedItems.Count - 1 To 0 Step -1
        ...
        If TypeResult = "New" Then
            For x = Me.lvList.CheckedItems.Count - 1 To 0 Step -1
                'MessageBox.Show(lvList.CheckedItems(x).SubItems(6).Text)
                rs.Parameters.Clear()
