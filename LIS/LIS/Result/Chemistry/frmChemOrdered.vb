...
Public Class frmChemOrdered
    ...
    Public Sub LoadTest()
        ...
        Try
            ...
            Next
            LoadRangeAndValues()
            loadAutomaticResult() 'Version 0.5.6.7
        Finally
    ...
    'Updates Version 0.5.6.7
    'For Static Computation of LDL and VLDL
    Public Sub loadAutomaticResult()
        Try
            Dim VLDL, LDL As Double 'Make this from string to Double v.1.5.10.8
            Dim TC, HDL, Trigly As String
            ...
            Connect()
            rs.Connection = conn
            rs.CommandText = "SELECT `universal_id`, `measurement`, `test_code` FROM `result`
                                WHERE `sample_id` LIKE @sample_id"
            reader = rs.ExecuteReader
            While reader.Read
                'Get the value of Trigly, HDL and Total Cholesterol
                If reader(2) = "Trigly" Then
                    Trigly = reader(1)
                ElseIf reader(2) = "C_HDL" Then
                    HDL = reader(1)
                ElseIf reader(2) = "C_Chol" Then
                    TC = reader(1)
                End If
            End While
            Disconnect()
            'Looping on Gridview to find the VLDL and LDL and Set the Automatic Computation
            For x As Integer = 0 To Me.GridView.RowCount - 1 Step 1
                'Fixed Version 1.5.10.8
                If Trigly = "" Then
                    Disconnect()
                    Exit Sub
                ElseIf HDL = "" Then
                ...
                ElseIf TC = "" Then
                    ...
                    Exit Sub
                End If
                'END OF Version 1.5.10.8
                VLDL = FormatNumber(Trigly / 5, 2)
                LDL = FormatNumber(TC - (HDL + VLDL))
                If GridView.GetRowCellValue(x, GridView.Columns("TestCode")) = "C_LDL" Then
                    GridView.SetRowCellValue(x, GridView.Columns("SI"), LDL)
                ElseIf GridView.GetRowCellValue(x, GridView.Columns("TestCode")) = "C_VLDL" Then
                    GridView.SetRowCellValue(x, GridView.Columns("SI"), VLDL)
                End If
            Next
        Catch ex As Exception
            Disconnect()
