...
Public Class RDLCPrinterPWorkSheet : Implements IDisposable
    ...
    Public Sub New(ByVal SampleID As String, ByVal User As String, ByVal DefaultPrinterName As String)
        'reportResourceName = "NameofProject.NameofRDLC"
        Dim reportResourceName = "SBSI_LIS_Software.ReportPatientWorkSheet.rdlc"
        ...
        myReport.LoadReportDefinition(Assembly.GetExecutingAssembly.GetManifestResourceStream(reportResourceName))
        ...
        Dim SQL = "SELECT * FROM `order` WHERE `main_id` LIKE '" & SampleID & "'"
        ...
        ''Result Table
        Connect()
        Dim result_SQL = "SELECT * FROM `patient_order` WHERE `sample_id` LIKE '" & SampleID & "'"
        ...
        '###################----Parameters----################################################################
        myReportParams = New List(Of ReportParameter)

        Dim Print_Status As New ReportParameter("CurrUser", User)
    ...
    Private Sub Export(ByVal report As LocalReport)
        Dim deviceInfo As String = _
          ...
          "  <MarginTop>0.2cm</MarginTop>" + _
          "  <MarginLeft>0.2cm</MarginLeft>" + _
