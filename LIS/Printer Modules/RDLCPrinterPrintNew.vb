...
Public Class RDLCPrinterPrintNew : Implements IDisposable
    ...
    Public Sub New(ByVal SampleID As String, ByVal Section As String, ByVal SubSection As String, ByVal PrintStatus As String, ByVal DefaultPrinterName As String)
        ...
        Dim SQL = "SELECT * FROM `tmpWorklist` WHERE `main_id` = '" & SampleID & "' AND testtype = '" & Section & "'"
        ...
        'Result Table
        Connect()
        Dim result_SQL = "SELECT * FROM `tmpresult` WHERE `sample_id` = '" & SampleID & "' AND `print_status` = '1' AND section = '" & Section & "' AND `accepted_result` = 1 ORDER BY `order_no`"
        ...
        Dim TotalDiffCount As New ReportParameter("TotalDiffCount", frmHemaNew.lblDiffCount.Text)
